dp_one <- function(N_subj = 100,T_obs  = 10, BB = 1000, Nv = 100, alpha  = 5, doses  = c(6, 8, 10) , seed = seed){
  set.seed(seed)
  ...
  or_init <- geeglm(logy ~ ns(gps_hat, 3) + d, id = id, data = sim.data, family = gaussian, corstr = "independence")
  ...
  for (b in 1:BB){
    ...
    for (j in seq_along(subj_idx)){
      ...
      if (runif(1) < alpha / (alpha + length(ids))){
        tmp$logy <- rnorm(nrow(tmp), tmp$pred, 0.5)
      }
    ...
    ## COV-DP
    or_bb <- geeglm(logy ~ ns(gps_bb, 3) + d, id = id, data = newdata, weights = newdata$w_dp, family = gaussian, corstr = "independence")
    ...
    or_wbb <- geeglm( logy ~ ns(d, 3), id = id, data = newdata, weights = newdata$w_dp * sw, family = gaussian, corstr = "independence")
    
    ## Store posterior means at fixed doses
    for (k in seq_along(doses)){
      cov_dp[b, k] <- mean(predict(or_bb, newdata = data.frame(d = rep(doses[k],nrow(newdata)), 
                                                               gps_bb=dnorm(doses[k],ps_bb$fitted.values,sigma_bb))))
      wor_dp[b, k] <- mean(predict(or_wbb, newdata = data.frame(d = doses[k])))
    }
  }
